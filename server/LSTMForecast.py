...
def encodeLabel(self, index)
    # integer encode direction
    encoder = LabelEncoder()
    self.values[:, index]encoder.fit_transform(self.values[:, index])

#values[:, 4] = encodeLabel(values[:, 4])
...
def dropColumns(self, columnIndexes):
    self.values.drop(self.values.columns[columnIndexes], axis=1, inplace=True)  

# drop columns we don't want to predict
#reframed = dropColumns([9,10,11,12,13,14,15], 1, True)
...
def reshapeTrainTestSetsTo3d(self):
    self.trainInput = self.trainInput.reshape((trainInput.shape[0], 1, trainInput.shape[1]))
    self.testInput = self.testInput.reshape((testInput.shape[0], 1, testInput.shape[1]))
...
def fitModel(self, filePath):
    self.history = self.model.fit(trainInput, trainOutput, epochs=50, batch_size=72, validation_data=(testInput, testOutput), verbose=2, shuffle=False) 
...
def getNormalizedPrediction(self, predictionInput):
    predictionInput = self.reshapePredictionInputTo3d(predictionInput)
...
def getActualPrediction(self, predictionInput, prediction):
    ...
    actualizedPrediction = concatenate((prediction, predictionInput[:, 1:]), axis=1)
    actualizedPrediction = self.scaler.inverse_transform(actualizedPrediction)
    return actualizedPrediction[:,0]

def train(self, filePath, encodeIndexes, columnsToDrop)
    # encode potential categorical columns
    if encodeIndexes:
        for i in encodeIndexes:
            self.encodeLabel(i)
    ...
    self.splitTrainTestSets(0.8)
    # split train and test sets to representative inputs and outputs
    self.splitInputOutput()
    self.reshapeTrainTestSetsTo3d()
    # prepare, fit, and save model
    self.initModel()
    self.fitModel(filePath)

def init(train=False, filePath, encodeIndexes, columnsToDrop):
    if train:
        self.train(filePath, encodeIndexes, columnsToDrop)
    else:
...
def predict(self, inputData):
    normalizedPrediction = self.getNormalizedPrediction(inputData)
    actualPrediction = self.getActualPrediction(inputData, normalizedPrediction)
